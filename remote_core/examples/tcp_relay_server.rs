...
async fn main() -> Result<(), Box<dyn Error + Send + Sync>> {
    let bind_addr = env_socket_addr(
        ...
        SocketAddr::from(([0, 0, 0, 0], 39491)),
    )?;
    let server = BoundTcpRelayServer::bind(
        TcpRelayServerConfig::new(bind_addr)
            .with_event_logging(env_flag_or("REMOTE_PLAY_RELAY_LOG", false)),
    )
    .await?;
    println!("RemotePlay TCP relay listening on {}", server.local_addr()?);
