use remote_core::relay::{BoundTcpRelayTunnel, TcpRelayTunnelConfig};
use std::error::Error;
...
async fn main() -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    config = config.with_event_logging(env_flag_or("REMOTE_PLAY_RELAY_LOG", false));

    let tunnel = BoundTcpRelayTunnel::bind(config).await?;
    println!(
        "RemotePlay TCP relay tunnel listening on {}",
        tunnel.local_addr()?
    );
